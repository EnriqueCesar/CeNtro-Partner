...
const groups = [
  { label: 'Partner', count: 7 },
  { label: 'Cliente', count: 5 },
  { label: 'Negocio', count: 6 },
] as const
...
export function RankingPage() {
  const { data, stores, stage, error, retry, dm, setDm, dms } = useData()
  ...
  return <>
    <section className="mb-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
      ...
      <StatCard label="Total de indicadores" value={data?.indicatorCount ?? 18} icon={ListChecks} />
    ...
    <section className="card mb-5 py-4">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <h2 className="section-title">Resultados YTD</h2>
        </div>
        <label className="filter-label w-full sm:w-72">Distrito
        ...
        <table className="ranking-table">
          <thead>
            <tr className="group-row">
              ...
              {groups.map(group => <th key={group.label} colSpan={group.count} className={`group-${group.label.toLowerCase()}`}>{group.label}</th>)}
              <th colSpan={1} className="group-gestion">Gestión</th>
            </tr>
            <tr>
              {(data?.stores[0]?.indicators ?? []).map(indicator => <th key={indicator.indicator}>{indicator.indicator}</th>)}
              <th>Cumplimiento</th>
            </tr>
          </thead>
          <tbody>
